...
if (!isset($params[2]) || $params[2] == 'today'){
    ...
    $title = 'Today';
    $sentence = 'You\'ve used <span class="hi today">14</span> today.';
} else {
    $date = $params[2];
    $t = strtotime($date);
    $title = 'for '.date('jS F',$t);
    $sentence = 'You used <span class="hi today">14</span> on this day.';
}

?>

<h2 class="page-title"><span class="icon icon-water icon--heading"></span>Electricity <?php echo $title; ?></h2>

<div class="module coloured colours-blue usage__graph__today">
    <h2 class="heading">So Far Today</h2>    
    <p class="m--half"><?php echo $sentence; ?> Let's see when you used most:</p>
...
<script type="text/javascript">
    ...
    $(document).on('ready',function()
    {
        ...
        $.get('/api/electricity/day/<?php echo $date; ?>/', {}, function(data, textStatus, jqXHR)
        {
            ...
            $('.usage__graph__today .hi').text(Math.round(100*todayTotal)/100+' kWh');
        ...
        function doDayComparison()
        {
            $.get('/api/electricity/day/average/<?php echo $date; ?>/', {}, function(data, textStatus, jqXHR)
            {
                ...
                compareGraph.setType('electricity');
        ...
        var piedata = [
            ...
            ,{label:'TV',num:23}
            ,{label:'heating',num:76}
            ,{label:'dish washer',num:54}
            ,{label:'   ',num:45}
